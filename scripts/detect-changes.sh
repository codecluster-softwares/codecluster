...
if [ -z "$BASE_BRANCH" ] || [ -z "$HEAD_REF" ]; then
  red "Error: Both base_branch and head_ref are required" >&2
  exit 1
fi
...
if [ -z "$GITHUB_OUTPUT" ]; then
  cyan "Changed files:" >&2
  dim "$(git diff --name-only "$BASE_BRANCH" "$HEAD_REF")" >&2
  magenta "Rust changes: $HAS_RUST_CHANGES" >&2
  yellow "Test changes: $HAS_TEST_CHANGES" >&2
  green "Frontend changes: $HAS_FRONTEND_CHANGES" >&2
fi
